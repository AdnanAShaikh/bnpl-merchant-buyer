...
export const fetchMarketplaceMerchants = createAsyncThunk<
  MarketplaceMerchantsResponse,
  void,
  { rejectValue: string }
>("buyer/fetchMarketplaceMerchants", async (_, { rejectWithValue }) => {
  try {
...
export const fetchMarketplaceMerchantProducts = createAsyncThunk<
  MarketplaceMerchantProductsResponse,
  number,
  { rejectValue: string }
>(
  ...
  async (merchantId, { rejectWithValue }) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.message || "Failed to fetch merchant products");
      return data;
    } catch (err: any) {
      return rejectWithValue(err.message);
    }
...
const buyerSlice = createSlice({
  ...
  reducers: {
    ...
    clearMerchantProductsError: (state) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchMarketplaceMerchants.pending, (state) => {
        state.merchantsLoading = true;
        state.merchantsError = null;
      })
      .addCase(fetchMarketplaceMerchants.fulfilled, (state, action) => {
        state.merchantsLoading = false;
        state.merchants = action.payload.merchants;
      })
      .addCase(fetchMarketplaceMerchants.rejected, (state, action) => {
        state.merchantsLoading = false;
        state.merchantsError = action.payload ?? "Something went wrong";
      });

    // ── One Merchant's Catalogue ──
    builder
      .addCase(fetchMarketplaceMerchantProducts.pending, (state) => {
      ...
      .addCase(fetchMarketplaceMerchantProducts.rejected, (state, action) => {
        state.merchantProductsLoading = false;
        state.merchantProductsError = action.payload ?? "Something went wrong";
      });
  },
});
...
// ─── Selectors ────────────────────────────────────────────────────────────────
export const selectMarketplaceMerchants = (state: { buyer: BuyerState }) =>
  state.buyer.merchants;
export const selectMarketplaceMerchantsLoading = (state: {
  buyer: BuyerState;
}) => state.buyer.merchantsLoading;
export const selectMarketplaceMerchantsError = (state: { buyer: BuyerState }) =>
  state.buyer.merchantsError;

export const selectActiveMerchant = (state: { buyer: BuyerState }) =>
  state.buyer.activeMerchant;
export const selectMerchantProducts = (state: { buyer: BuyerState }) =>
  state.buyer.merchantProducts;
export const selectMerchantProductsLoading = (state: { buyer: BuyerState }) =>
  state.buyer.merchantProductsLoading;
export const selectMerchantProductsError = (state: { buyer: BuyerState }) =>
  state.buyer.merchantProductsError;
