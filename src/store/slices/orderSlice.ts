...
import type { OrderDetail } from "../../screens/buyer/Orders/BuyerOrderViewScreen";
import type { MerchantOrderDetail } from "../../screens/merchant/Orders/MerchantOrderViewScreen";
...
export const fetchBuyerOrderById = createAsyncThunk<
  ...
  number,
  { rejectValue: string }
>("order/fetchBuyerOrderById", async (id, { rejectWithValue }) => {
  try {
    const res = await apiFetch(`/api/order/buyer/${id}`);
    const data = await res.json();
...
export const fetchMerchantOrderById = createAsyncThunk<
  { order: MerchantOrderDetail },
  number,
  { rejectValue: string }
>("order/fetchMerchantOrderById", async (id, { rejectWithValue }) => {
  try {
    const res = await apiFetch(`/api/order/merchant/${id}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || "Failed to load order");
    return data;
  } catch (err: any) {
...
export const confirmOrder = createAsyncThunk<
  ...
  ConfirmOrderPayload,
  { rejectValue: string }
>("order/confirmOrder", async ({ id }, { rejectWithValue }) => {
  try {
...
export const declineOrder = createAsyncThunk<
  ...
  DeclineOrderPayload,
  { rejectValue: string }
>("order/declineOrder", async ({ id, declineReason }, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchMyOrders.rejected, (state, action) => {
        state.myOrdersLoading = false;
        state.myOrdersError = action.payload ?? "Something went wrong";
      });
    ...
    builder
      ...
      .addCase(placeOrder.rejected, (state, action) => {
        state.actionLoading = false;
        state.actionError = action.payload ?? "Something went wrong";
      });
...
export const selectOrdersLoading = (state: { order: OrderState }) =>
  state.order.ordersLoading;
export const selectOrdersError = (state: { order: OrderState }) =>
  state.order.ordersError;

export const selectSingleLoading = (state: { order: OrderState }) =>
  state.order.singleLoading;
export const selectSingleError = (state: { order: OrderState }) =>
  state.order.singleError;

export const selectActionLoading = (state: { order: OrderState }) =>
  state.order.actionLoading;
export const selectActionError = (state: { order: OrderState }) =>
  state.order.actionError;
