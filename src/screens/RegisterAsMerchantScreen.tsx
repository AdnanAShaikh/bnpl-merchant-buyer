...
import { emailOtpSendAndVerify, registerMerchant, selectEmailOtpLoading } from "../store/slices/authSlice";
...
const COUNTRIES = [
  ...
  "Egypt", "Jordan", "Lebanon", "Iraq", "Yemen", "Syria", "Palestine",
  "India", "Pakistan", "Bangladesh", "Philippines", "United Kingdom",
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Input
        ...
        required value={data.email} onChange={(v) => onChange("email", v)}
        error={errors.email}
      />
      <Input
        label="Create Password" name="password" placeholder="Please enter"
        required value={data.password} onChange={(v) => onChange("password", v)}
      ...
      <Input
        ...
        required value={data.confirmPassword} onChange={(v) => onChange("confirmPassword", v)}
...
const CompanyDetails = ({ data, onChange, errors }: any) => (
  <div className="flex flex-col gap-4">
    ...
    {/* Company Type */}
    <SelectField
      label="Company Type" name="companyType" required
      value={data.companyType} onChange={(v) => onChange("companyType", v)}
    ...
    <div className="grid grid-cols-2 gap-4">
      <Input label="Company Name" name="companyName" required
        value={data.companyName} onChange={(v) => onChange("companyName", v)} error={errors.companyName} />
      <SelectField
        label="Company Presence" name="companyPresence" required
        value={data.companyPresence} onChange={(v) => onChange("companyPresence", v)}
    ...
    <div className="grid grid-cols-2 gap-4">
      <Input label="Corporate Telephone Number" name="corporateTelephone" type="tel" required
        value={data.corporateTelephone} onChange={(v) => onChange("corporateTelephone", v)} error={errors.corporateTelephone} />
      <Input label="Company Registration Number" name="companyRegistrationNo" required
        value={data.companyRegistrationNo} onChange={(v) => onChange("companyRegistrationNo", v)} error={errors.companyRegistrationNo} />
    </div>

    {/* Annual Turnover + Number of Employees */}
    <div className="grid grid-cols-2 gap-4">
      <Input label="Business Annual Turnover (SAR)" name="annualTurnover" type="number" required
        value={data.annualTurnover} onChange={(v) => onChange("annualTurnover", v)} error={errors.annualTurnover} />
      <Input label="Number of Employees" name="numberOfEmployees" type="number" required
        value={data.numberOfEmployees} onChange={(v) => onChange("numberOfEmployees", v)} error={errors.numberOfEmployees} />
    </div>

    {/* Operation License + Expiry */}
    <div className="grid grid-cols-2 gap-4">
      <Input label="Operation License Number" name="operationLicenseNo"
        value={data.operationLicenseNo || ""} onChange={(v) => onChange("operationLicenseNo", v)} />
      <Input label="Expiration Date" name="operationLicenseExpiry" type="date"
        value={data.operationLicenseExpiry || ""} onChange={(v) => onChange("operationLicenseExpiry", v)} />
    </div>

    {/* SAGIA Number */}
    <Input label="SAGIA Number" name="sagiaNumber"
      value={data.sagiaNumber || ""} onChange={(v) => onChange("sagiaNumber", v)} />
  ...
  <div className="flex flex-col gap-4">
    ...
    <div className="grid grid-cols-2 gap-4">
      <SelectField
        ...
        value={data.title} onChange={(v) => onChange("title", v)}
        error={errors.title}
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <SelectField
        label="Nationality" name="nationality" required
        value={data.nationality} onChange={(v) => onChange("nationality", v)}
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <Input label="Date of Birth" name="dateOfBirth" type="date" required
        value={data.dateOfBirth} onChange={(v) => onChange("dateOfBirth", v)} error={errors.dateOfBirth} />
    </div>

    {/* Place of Birth */}
    <SelectField
      label="Place of Birth" name="placeOfBirth" required
      value={data.placeOfBirth} onChange={(v) => onChange("placeOfBirth", v)}
  ...
  <div className="flex flex-col gap-5">
    ...
    <div>
      ...
      <p className="text-sm text-amber-700 mt-1 leading-relaxed">
        Your form progress is saved automatically, but uploaded files
        will be lost if the page is refreshed. You may need to upload
        them again.
      </p>
    </div>
    {[
      ...
      { documentType: "power_of_attorney_representative", label: "Power of Attorney Representative ID",           hint: undefined, required: true },
      ...
      { documentType: "bank_statements",              label: "Bank Issued and Certified IBAN Letter",                  hint: "Period of last 6 months.",   required: true },
    ...
    <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <SelectField
            ...
            value={data.countryCode} onChange={(v) => onChange("countryCode", v)}
  ...
  const sections = [
    ...
    },
    {
      ...
      rows: [
        ...
        { label: "Registration No.",      value: allData.company.companyRegistrationNo  },
        { label: "Annual Turnover (SAR)", value: allData.company.annualTurnover         },
        ...
        { label: "License Expiry",        value: allData.company.operationLicenseExpiry || "—" },
    ...
    },
    {
      ...
      rows: [
        ...
        { label: "Invoicing Mobile", value: `${allData.gnpl.invoicingCountryCode} ${allData.gnpl.invoicingMobile}` },
...
function validateStep(step: number, data: any): Record<string, string> {
  ...
  if (step === 1) {
    ...
    if (!data.companyPresence)       errs.companyPresence       = "Company presence is required";
    if (!data.annualTurnover)        errs.annualTurnover        = "Annual turnover is required";
    if (!data.numberOfEmployees)     errs.numberOfEmployees     = "Number of employees is required";

    // Conditional — only required if presence is Online or Both
    if ((data.companyPresence === "Online" || data.companyPresence === "Both") && !data.ecommerceUrl) {
  ...
  // ── Step 4: GNPL Details ──
  if (step === 4) {
    if (!data.invoicingEmail) errs.invoicingEmail = "Invoicing email is required";
    else if (!/\S+@\S+\.\S+/.test(data.invoicingEmail)) errs.invoicingEmail = "Enter a valid email";
    if (!data.invoicingMobile) errs.invoicingMobile = "Invoicing mobile is required";
    if (!data.invoicingCountryCode) errs.invoicingCountryCode = "Country Code is required";
...
const RegisterAsMerchantScreen = () => {
  ...
  const handleNext = async () => {
    ...
    const result = await dispatch(registerMerchant({
      profileData: {
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-24">
            <Stepper
              ...
              sx={{
                ...
                "& .MuiStepLabel-label.Mui-active": { color: NAVY, fontWeight: 600 },
                "& .MuiStepLabel-label.Mui-completed": { color: RED, fontWeight: 500 },
        ...
        <div className="flex-1">
          <div className="bg-white rounded-2xl border border-gray-100 p-8">
            ...
            {activeStep === 3 &&  <SupportingDocuments
                                  ...
                                  />}
            {activeStep === 4 && <GNPLDetails data={gnplData} onChange={patchGNPL} errors={errors} />}
            {activeStep === 5 && (
              <ReviewConfirm allData={{ profile: profileData, company: companyData, attorney: attorneyData, gnpl: gnplData}} />
            )}
