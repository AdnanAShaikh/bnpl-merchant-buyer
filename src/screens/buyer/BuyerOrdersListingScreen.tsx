...
const STATUS_META: Record<string, { label: string; cls: string; dot: string; desc: string }> = {
  PENDING_REVIEW: { label: "Pending Review", cls: "bg-amber-400 text-gray-800", dot: "bg-amber-400",   desc: "Your financing request has been submitted and is waiting for Tabashir to begin review." },
  UNDER_REVIEW:   { label: "Under Review",   cls: "bg-blue-500",                dot: "bg-blue-500",    desc: "A Tabashir reviewer is assessing your request and finalising the terms." },
  APPROVED:       { label: "Approved",       cls: "bg-teal-600",                dot: "bg-teal-600",    desc: "Tabashir has approved your request and is arranging the purchase with the merchant." },
  ACTIVE:         { label: "Active",         cls: "bg-emerald-600",             dot: "bg-emerald-600", desc: "Financing is active — goods are being dispatched and your installments have begun." },
  COMPLETED:      { label: "Completed",      cls: "bg-primary",               dot: "bg-primary",   desc: "All installments have been paid and the order is fully settled." },
  REJECTED:       { label: "Rejected",       cls: "bg-red-500",                 dot: "bg-red-500",     desc: "Your request was not approved. Check the order details for the reason provided." },
  CANCELLED:      { label: "Cancelled",      cls: "bg-gray-500",                dot: "bg-gray-500",    desc: "The request was cancelled before approval — either by you or by Tabashir." },
  DEFAULTED:      { label: "Defaulted",      cls: "bg-red-700",                 dot: "bg-red-700",     desc: "One or more installments were missed and the order has entered default." },
};
...
const freqNoun = (t: string) => {
  const map: Record<string, string> = { Daily: "day", Weekly: "week", Biweekly: "2 wks", Monthly: "mo" };
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">What each stage of your order means</p>
          </div>
...
const BuyerOrdersListingScreen = () => {
  ...
  const stats = useMemo(() => {
    ...
    const pending   = orders.filter((o: Order) => o.status === "PENDING_REVIEW" || o.status === "UNDER_REVIEW");
    ...
    { key: "totalAmount", label: "Total", render: (v, row) => money(v as string, row.currency) },
    {
      ...
      render: (v, row) => (
        <span>
          ...
          <span className="text-gray-400"> / {freqNoun(row.installmentFrequency)}</span>
        </span>
      ),
    },
  ...
  const ROW_ACTIONS: RowAction<Order>[] = [
    { label: "View Order", icon: eyeIcon, onClick: (order) => navigate(`/buyer/orders/${order.id}`) },
    {
      label: "Cancel Order",
      icon: trashIcon,
        ...
        >
          ...
          >
            <div className="px-6 py-5">
              ...
              <h3 className="text-base font-bold text-primary mb-1">Cancel this order?</h3>
              <p className="text-sm text-gray-500 leading-relaxed">
                Order <span className="font-semibold text-primary">#{cancelTarget.id}</span>
                {cancelTarget.product?.name ? ` for ${cancelTarget.product.name}` : ""} will be cancelled.
