    ...
    <div className="flex flex-col gap-6">
      <Section title="Company">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col gap-1 w-full">
            <div className="border border-gray-100 rounded-xl px-4 pt-2.5 pb-2 bg-gray-50">
              <label className="text-xs text-gray-400 font-medium">Online Store</label>
              {showUrl ? (
                
                <a href={cd.ecommerceUrl.startsWith("http") ? cd.ecommerceUrl : `https://${cd.ecommerceUrl}`}
                  target="_blank"
...
const ProductCard = ({ product, onClick }: { product: Product; onClick: () => void }) => (
  ...
  >
    ...
    <div className="p-4">
      <p className="text-sm font-bold text-[#1a2a4a] truncate">{product.name}</p>
      <p className="text-xs text-gray-400 mt-0.5 truncate">{product.category}</p>
      <p className="text-sm font-semibold text-[#1a2a4a] mt-2">{money(product.price, product.currency)}</p>
    </div>
  </button>
);
  ...
  if (products.length === 0) {
    return (
      <div className="py-16 text-center">
        <p className="text-sm font-semibold text-[#1a2a4a] mb-1">No products yet</p>
        <p className="text-sm text-gray-400">This merchant hasn't listed any active products.</p>
      </div>
    );
  }
...
const BuyerMerchantsViewScreen = () => {
  ...
  useEffect(() => {
    ...
    return () => { dispatch(clearActiveMerchant()); };
  }, [id, dispatch]);
    ...
    return (
      <SidebarBuyer>
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-3 text-sm text-gray-400">Loading merchant...</span>
        </div>
      </SidebarBuyer>
    );
  }
    ...
    return (
      <SidebarBuyer>
        <div className="flex flex-col items-center justify-center h-64 text-center">
          <p className="text-sm font-semibold text-[#1a2a4a] mb-1">Couldn't load this merchant</p>
          <p className="text-sm text-gray-400 mb-4">{error || "Merchant not found or not available."}</p>
          <button
    ...
    <SidebarBuyer>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#1a2a4a]">{companyName}</h1>
