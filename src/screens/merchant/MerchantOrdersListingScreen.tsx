...
interface Order {
  ...
  requestedPlan?: { id: number; planName: string; termType: string; termValue: number };
}

// ─── Status display + meaning ─────────────────────────────────────────────────
const STATUS_META: Record<string, { label: string; cls: string; dot: string; desc: string }> = {
  PENDING_REVIEW: { label: "Pending Review", cls: "bg-amber-400 text-gray-800", dot: "bg-amber-400",   desc: "The buyer has requested financing. Tabashir is beginning its review." },
  UNDER_REVIEW:   { label: "Under Review",   cls: "bg-blue-500",                dot: "bg-blue-500",    desc: "Tabashir is assessing the request and finalising terms with the buyer." },
  APPROVED:       { label: "Approved",       cls: "bg-teal-600",                dot: "bg-teal-600",    desc: "Tabashir has approved the order. Please confirm you can fulfil it." },
  ACTIVE:         { label: "Active",         cls: "bg-emerald-600",             dot: "bg-emerald-600", desc: "The order is active — goods should be dispatched to the buyer." },
  COMPLETED:      { label: "Completed",      cls: "bg-primary",               dot: "bg-primary",   desc: "The order has been fully settled and closed." },
  REJECTED:       { label: "Rejected",       cls: "bg-red-500",                 dot: "bg-red-500",     desc: "Tabashir did not approve this financing request." },
  CANCELLED:      { label: "Cancelled",      cls: "bg-gray-500",                dot: "bg-gray-500",    desc: "The request was cancelled before it became active." },
  DEFAULTED:      { label: "Defaulted",      cls: "bg-red-700",                 dot: "bg-red-700",     desc: "The buyer has defaulted on their installments." },
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-base font-bold text-primary">Order Statuses Explained</h3>
            <p className="text-xs text-gray-400 mt-0.5">What each stage means for you</p>
          </div>
...
const MerchantOrdersListingScreen = () => {
  ...
  const [respondMode, setRespondMode]     = useState<"confirm" | "decline">("confirm");
  ...
  const stats = useMemo(() => {
    ...
    const awaiting   = orders.filter((o: Order) => o.status === "APPROVED" && !o.merchantConfirmed && !o.merchantDeclined);
  ...
  const handleRespond = async () => {
    ...
    setResponding(true);
    const result = respondMode === "confirm"
    ...
    if (thunk.fulfilled.match(result)) {
      toast.success(respondMode === "confirm" ? "Order confirmed" : "Order declined");
  ...
  const COLUMNS: ColumnDef<Order>[] = [
    { key: "id", label: "Order #", render: (v) => <span className="font-semibold text-primary">#{String(v)}</span> },
    ...
    { key: "totalAmount", label: "Order Value", render: (v, row) => money(v as string, row.currency) },
    {
      key: "status",
      label: "Status",
      render: (value) => {
    ...
    },
    {
      ...
      render: (_v, row) => {
        ...
        return <span className={`inline-block text-xs font-bold px-3 py-1 rounded-md text-white ${r.cls}`}>{r.label}</span>;
      },
    },
  ...
  const canRespond = (o: Order) =>
    RESPONDABLE_STATUSES.includes(o.status) && !o.merchantConfirmed && !o.merchantDeclined;

  const ROW_ACTIONS: RowAction<Order>[] = [
    { label: "View Order", icon: eyeIcon, onClick: (order) => navigate(`/merchant/orders/${order.id}`) },
    {
    ...
    <SidebarMerchant>
      ...
      <div className="flex gap-5 mb-6">
        <div className="grid grid-cols-2 gap-4 flex-1">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <p className="text-2xl font-bold text-primary">{stats.awaitingCount}</p>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
      ...
      {error && !loading && (
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
          <p className="text-sm font-semibold text-primary mb-1">Couldn't load orders</p>
          <p className="text-sm text-gray-400 mb-4">{error}</p>
          <button onClick={() => dispatch(fetchMyOrders())}
      ...
      {!loading && !error && (
        <DataTable<Order>
          ...
          showStatusFilter
          statusOptions={["PENDING_REVIEW", "UNDER_REVIEW", "APPROVED", "ACTIVE", "COMPLETED", "REJECTED", "CANCELLED", "All Status"]}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden"
            ...
            <div className="px-6 py-5">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${respondMode === "confirm" ? "bg-green-50" : "bg-red-50"}`}>
                {respondMode === "confirm" ? (
              ...
              <h3 className="text-base font-bold text-primary mb-1">
                {respondMode === "confirm" ? "Confirm fulfillment?" : "Decline this order?"}
              </h3>
              <p className="text-sm text-gray-500 leading-relaxed mb-3">
                Order <span className="font-semibold text-primary">#{respondTarget.id}</span>
                {respondTarget.product?.name ? ` for ${respondTarget.product.name}` : ""}
                {respondMode === "confirm"
              ...
              {respondMode === "decline" && (
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500 font-medium">Reason for declining <span className="text-red-500">*</span></label>
                  <textarea
                    value={declineReason}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
              <button onClick={() => setRespondTarget(null)} disabled={responding}
              ...
              <button onClick={handleRespond} disabled={responding}
                ...
                ) : respondMode === "confirm" ? "Confirm" : "Decline"}
              </button>
            </div>
          </div>
        </div>
